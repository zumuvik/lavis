use super::{
    ...
    protocol::{self, MessageEvent, MessageEventKind, V6CallError, V6InboundFrame, V6ModuleFrame, V6OutboundCoreFrame},
    v6_executor::{V6ExecutionContext, V6ExecutorError, V6TelegramExecutor},
    v6_registry,
};
use crate::error::ExternalError;
use std::{collections::{HashMap, HashSet}, ffi::OsString, future, path::{Path, PathBuf}, process::Stdio, sync::Arc};
use tokio::{
...
impl V6Process {
    pub(crate) async fn start(
        ...
        if descriptor.protocol_version != 6 || !descriptor.entrypoint.starts_with(&descriptor.module_dir) {
        ...
        tokio::fs::create_dir_all(&state_dir).await.map_err(|_| ExternalError::StateWrite)?;
        secure_directory(&state_dir).await.map_err(|_| ExternalError::StateWrite)?;
        ...
        let Some(process_group) = child.id() else { cleanup_spawned_child(&mut child, None).await; return Err(ExternalError::Unavailable); };
        let Some(stdin) = child.stdin.take() else { cleanup_spawned_child(&mut child, Some(process_group)).await; return Err(ExternalError::Unavailable); };
        let Some(stdout) = child.stdout.take() else { cleanup_spawned_child(&mut child, Some(process_group)).await; return Err(ExternalError::Unavailable); };
        let Some(stderr) = child.stderr.take() else { cleanup_spawned_child(&mut child, Some(process_group)).await; return Err(ExternalError::Unavailable); };
        ...
        tokio::spawn(supervise(
            child, process_group, descriptor.clone(), executor, SupervisorIo { control_rx, reader_rx, writer_tx, rpc_rx, actor_tx: rpc_tx, reader, writer, stderr_drain },
        ));
        Ok(Self {
            control: control_tx,
        ...
        match frame {
            V6InboundFrame::EventResult { actions, .. } => {
                ...
                match protocol::parse_module_line_for(&line, 6)? {
                    Some(protocol::ModuleMessage::EventResult { request_id, actions }) => {
    ...
    pub(crate) async fn initialize(&self, request_id: String, module_id: String) -> Result<V6InboundFrame, ExternalError> {
        self.request(V6OutboundCoreFrame::Initialize { request_id, module_id }, Expected::Initialized).await
    }
    ...
    pub(crate) async fn execute(
        ...
        self.request(V6OutboundCoreFrame::Execute { request_id, command, arguments, argument_entities }, Expected::Result).await
    }

    pub(crate) async fn event(&self, request_id: String, event: MessageEventKind, payload: MessageEvent) -> Result<V6InboundFrame, ExternalError> {
        self.request(V6OutboundCoreFrame::Event { request_id, event, payload }, Expected::EventResult).await
    }

    pub(crate) async fn shutdown(&self, request_id: String) -> Result<(), ExternalError> {
        let (reply, response) = oneshot::channel();
        self.control.send(Control::Shutdown { request_id, reply }).await.map_err(|_| ExternalError::Unavailable)?;
        response.await.map_err(|_| ExternalError::Unavailable)?
    }

    async fn request(&self, frame: V6OutboundCoreFrame, expected: Expected) -> Result<V6InboundFrame, ExternalError> {
        let (reply_tx, reply_rx) = oneshot::channel();
        self.control.send(Control::Request { frame, expected, reply: reply_tx }).await.map_err(|_| ExternalError::Unavailable)?;
        reply_rx.await.map_err(|_| ExternalError::Unavailable)?
    }
}

enum Control {
    Request { frame: V6OutboundCoreFrame, expected: Expected, reply: oneshot::Sender<Result<V6InboundFrame, ExternalError>> },
    Shutdown { request_id: String, reply: oneshot::Sender<Result<(), ExternalError>> },
}

#[derive(Clone, Copy, PartialEq, Eq)]
enum Expected { Initialized, Health, Result, EventResult }

struct Pending { expected: Expected, deadline: Instant, reply: oneshot::Sender<Result<V6InboundFrame, ExternalError>> }

enum WriterCommand {
    Frame(V6OutboundCoreFrame, Flush),
}

enum Flush { None, Call(String), Shutdown }

enum ActorEvent {
    Inbound(Result<V6InboundFrame, ExternalError>),
    RpcComplete { call_id: String, result: Result<serde_json::Value, V6CallError> },
    Flushed(String),
    ShutdownFlushed,
    WriterFailed,
    ReaderEof,
...
async fn supervise(
    ...
    let SupervisorIo { mut control_rx, mut reader_rx, writer_tx, mut rpc_rx, actor_tx, mut reader, mut writer, mut stderr_drain } = io;
    ...
    if !child_reaped {
        if force_kill || timeout(V6_SHUTDOWN_TIMEOUT, child.wait()).await.is_err() { kill_group(process_group); }
...
fn start_shutdown(
    ...
    *closing = true;
    ...
    workers.abort_all();
    if queue_writer(writer, WriterCommand::Frame(V6OutboundCoreFrame::Shutdown { request_id }, Flush::Shutdown)).is_err() {
        if let Some(reply) = reply { let _ = reply.send(Err(ExternalError::Unavailable)); }
        return false;
    }
...
async fn read_stdout(mut stdout: BufReader<tokio::process::ChildStdout>, tx: mpsc::Sender<ActorEvent>) {
    ...
    loop {
        ...
        loop {
            match stdout.read(&mut byte).await {
                Ok(0) => { let _ = tx.send(ActorEvent::ReaderEof).await; return; }
                Ok(_) if byte[0] == b'\n' => break,
                Ok(_) if line.len() >= protocol::MAX_LINE_BYTES => { let _ = tx.send(ActorEvent::Inbound(Err(ExternalError::LineTooLarge))).await; return; }
                Ok(_) => line.push(byte[0]),
                Err(_) => { let _ = tx.send(ActorEvent::Inbound(Err(ExternalError::ProtocolDecode))).await; return; }
            }
        }
        let parsed = std::str::from_utf8(&line).map_err(|_| ExternalError::ProtocolDecode).and_then(protocol::parse_v6_inbound_frame);
        if tx.send(ActorEvent::Inbound(parsed)).await.is_err() { return; }
    }
}

async fn write_stdin(mut stdin: tokio::process::ChildStdin, mut rx: mpsc::Receiver<WriterCommand>, tx: mpsc::Sender<ActorEvent>) {
    while let Some(WriterCommand::Frame(frame, flush)) = rx.recv().await {
        let line = match frame.serialize() { Ok(line) => line, Err(_) => { let _ = tx.send(ActorEvent::WriterFailed).await; return; } };
        let write = async { stdin.write_all(line.as_bytes()).await?; stdin.write_all(b"\n").await?; stdin.flush().await };
        if !matches!(timeout(V6_WRITE_TIMEOUT, write).await, Ok(Ok(()))) {
            let _ = tx.send(ActorEvent::WriterFailed).await;
            return;
        }
        let event = match flush { Flush::None => None, Flush::Call(call_id) => Some(ActorEvent::Flushed(call_id)), Flush::Shutdown => Some(ActorEvent::ShutdownFlushed) };
        if let Some(event) = event { if tx.send(event).await.is_err() { return; } }
    }
}

fn validate_invoke(descriptor: &ExternalModuleDescriptor, method: &str, unique: bool) -> Result<v6_registry::V6Method, V6CallError> {
    if !unique { return Err(V6CallError { kind: "protocol".to_owned(), message: "duplicate call_id".to_owned() }); }
    if !descriptor.capabilities.contains(&ExternalCapability::TelegramRaw) { return Err(V6CallError { kind: "capability".to_owned(), message: "telegram.raw capability is required".to_owned() }); }
    let method = v6_registry::lookup(method).ok_or_else(|| V6CallError { kind: "validation".to_owned(), message: "method is not allowlisted".to_owned() })?;
    if !descriptor.telegram_methods.contains(&method) { return Err(V6CallError { kind: "capability".to_owned(), message: "method is not granted".to_owned() }); }
    Ok(method)
}

fn map_executor_result(result: Result<super::v6_executor::V6RpcOutput, V6ExecutorError>) -> Result<serde_json::Value, V6CallError> {
    result.map(|value| value.into_value()).map_err(v6_executor_error)
}

fn v6_executor_error(error: V6ExecutorError) -> V6CallError {
    match error {
        V6ExecutorError::InvalidParams(_) => V6CallError { kind: "validation".to_owned(), message: "invalid parameters".to_owned() },
        V6ExecutorError::Rpc { .. } => V6CallError { kind: "rpc".to_owned(), message: "Telegram RPC request failed".to_owned() },
        V6ExecutorError::Transport => V6CallError { kind: "transport".to_owned(), message: "Telegram transport unavailable".to_owned() },
        V6ExecutorError::InvalidResponse => V6CallError { kind: "internal".to_owned(), message: "invalid gateway response".to_owned() },
        V6ExecutorError::ShuttingDown => V6CallError { kind: "shutdown".to_owned(), message: "gateway is shutting down".to_owned() },
    }
}

fn request_id(frame: &V6OutboundCoreFrame) -> Option<&str> {
    match frame { V6OutboundCoreFrame::Initialize { request_id, .. } | V6OutboundCoreFrame::Execute { request_id, .. } | V6OutboundCoreFrame::Event { request_id, .. } | V6OutboundCoreFrame::Health { request_id } | V6OutboundCoreFrame::Shutdown { request_id } => Some(request_id), V6OutboundCoreFrame::TelegramResult { .. } => None }
}

fn terminal_request_id(frame: &V6InboundFrame) -> Option<&str> {
    match frame { V6InboundFrame::Initialized { request_id, .. } | V6InboundFrame::Result { request_id, .. } | V6InboundFrame::Error { request_id, .. } | V6InboundFrame::Health { request_id } | V6InboundFrame::EventResult { request_id, .. } => Some(request_id), V6InboundFrame::Log { .. } | V6InboundFrame::TelegramInvoke(_) => None }
}

fn expected_matches(expected: Expected, frame: &V6InboundFrame) -> bool {
    matches!((expected, frame), (Expected::Initialized, V6InboundFrame::Initialized { .. }) | (Expected::Health, V6InboundFrame::Health { .. }) | (Expected::Result, V6InboundFrame::Result { .. } | V6InboundFrame::Error { .. }) | (Expected::EventResult, V6InboundFrame::EventResult { .. } | V6InboundFrame::Error { .. }))
}

fn initialized_module_matches(descriptor: &ExternalModuleDescriptor, frame: &V6InboundFrame) -> bool {
...
fn shutdown_child_exit_result(
    ...
    if !flushed { return Err(ExternalError::Unavailable); }
    child_exit.map(|_| ()).map_err(|_| ExternalError::Unavailable)
}

fn reader_eof_is_fatal(closing: bool) -> bool { !closing }
...
fn fail_pending(pending: &mut HashMap<String, Pending>) { for (_, request) in pending.drain() { let _ = request.reply.send(Err(ExternalError::Unavailable)); } }

fn kill_group(process_group: u32) {
    #[cfg(unix)]
    unsafe { let _ = libc::kill(-(process_group as i32), libc::SIGKILL); }
    #[cfg(not(unix))]
    let _ = process_group;
}
    ...
    F: Fn(&str) -> Option<OsString>,
{
    let base = environment("XDG_STATE_HOME")
        ...
        .map(PathBuf::from)
        .or_else(|| environment("HOME").filter(|value| !value.is_empty()).map(|home| Path::new(&home).join(".local/state")))?;
    base.is_absolute().then_some(base.join("lavis/modules").join(module_id))
}
...
#[cfg(not(unix))]
async fn secure_directory(_path: &Path) -> std::io::Result<()> { Ok(()) }

async fn cleanup_spawned_child(child: &mut Child, pid: Option<u32>) {
    #[cfg(unix)]
    if let Some(pid) = pid {
        let ret = unsafe { libc::kill(-(pid as i32), libc::SIGKILL) };
        if ret == -1 && std::io::Error::last_os_error().raw_os_error() != Some(libc::ESRCH) { let _ = child.kill().await; }
    }
    #[cfg(not(unix))]
    let _ = pid;
    let _ = child.wait().await;
...
mod tests {
    ...
    fn descriptor() -> ExternalModuleDescriptor {
        ExternalModuleDescriptor { protocol_version: 6, id: "test".to_owned(), display_name: "Test".to_owned(), version: "1".to_owned(), author: "A".to_owned(), entrypoint: PathBuf::from("run"), module_dir: PathBuf::from("."), capabilities: vec![], default_command: None, subscriptions: vec![], telegram_methods: vec![], actions: vec![], commands: vec![] }
    }

    #[test]
    fn ungranted_method_is_rejected_before_executor() {
        let descriptor = descriptor();
        assert!(matches!(validate_invoke(&descriptor, "account.updateStatus", true), Err(V6CallError { kind, .. }) if kind == "capability"));
    }

    #[test]
    fn terminal_frames_preserve_their_request_id() {
        let frame = V6InboundFrame::Health { request_id: "12".to_owned() };
    ...
    fn initialized_module_id_must_match_descriptor() {
        let frame = V6InboundFrame::Initialized { request_id: "12".to_owned(), module_id: "other".to_owned() };
        assert!(!initialized_module_matches(&descriptor(), &frame));
    }

    #[test]
    fn executor_errors_are_sanitized() {
        assert_eq!(v6_executor_error(V6ExecutorError::InvalidParams("secret")), V6CallError { kind: "validation".to_owned(), message: "invalid parameters".to_owned() });
        assert_eq!(v6_executor_error(V6ExecutorError::Rpc { code: 420, name: "FLOOD_WAIT".to_owned(), retry_after_seconds: Some(7) }).kind, "rpc");
    }
    ...
    fn shutdown_requires_a_flushed_shutdown_frame_before_child_exit() {
        #[cfg(unix)]
        {
            ...
            assert!(matches!(shutdown_child_exit_result(false, exited), Err(ExternalError::Unavailable)));
        }
    ...
    fn pending_expiry_drains_expired_requests() {
        ...
        pending.insert("1".to_owned(), Pending { expected: Expected::Health, deadline: Instant::now() - Duration::from_secs(1), reply });
        expire_pending(&mut pending);
        assert!(pending.is_empty());
        assert!(matches!(receiver.try_recv(), Ok(Err(ExternalError::ExecutionTimeout))));
    }
    ...
    async fn pending_deadline_is_absolute_under_event_activity() {
        ...
        pending.insert("1".to_owned(), Pending { expected: Expected::Health, deadline: Instant::now(), reply });
        tokio::time::timeout(Duration::from_millis(20), sleep_until_pending_deadline(&pending))
            .await
            .unwrap();
    }
}
